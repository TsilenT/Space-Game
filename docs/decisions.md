...
- Ship maps are authored directly at 36x24 in `src/sim/map.ts` — one ASCII row per deck line, read 1:1 with no load-time scaling. (The maps were originally authored at 12x8 and tripled at load; that scaling step and its door special-casing were later removed in favour of full-size definition files.) Rooms read as real spaces several tiles across.
- Doors are edges, not tiles: a door occupies the boundary between two adjacent squares, so no unit can ever stand in a door. An authored door block becomes a one-tile-thick wall plane with a floor threshold as its opening; the door edge sits on the room-facing side of that threshold and is drawn on the grid line. Closed doors block movement, sight, and fire across the edge.
- Clicking a door sends the selected soldier walking up to it, then forces it open for free, revealing what lies behind; stepping into a closed door with the keyboard opens it the same way. Enemies spend one of their steps forcing a door. Every door swings shut again at the end of each side's turn, though explored map memory persists.
